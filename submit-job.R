...
if(1)
{
  ...
  args <- data.table(
    ...
    p_undiag_probs='0.1139,0.1729,0.1812,0.13880,0.2268',
    rho="infdate",
    pre="infdate",
    em="infdate",
    nonB=1,
...
for(i in seq_len(nrow(args)))
{
  ...
  tmp 			<- paste0('Rscript ', file.path(args$source_dir[i],args$script_file[i]), 
                   ...
                   ' -keep_prop_dead_chains ', args$keep_prop_dead_chains[i],
                   ' -rho1 ', args$rho1[i],
                   ' -infdate ', args$infdate[i],
                   ' -p_undiag ', args$p_undiag[i],
                   ' -p_undiag_b ', args$p_undiag_b[i],
                   ' -p_undiag_nb ', args$p_undiag_nb[i],
                   ' -p_undiag_gps ', args$p_undiag_gps[i],
                   ' -p_undiag_probs ', args$p_undiag_probs[i],
                   ' -rho ', args$rho[i],
                   ' -pre ', args$pre[i],
                   ' -em ', args$em[i],
                   ' -nonB ', args$nonB[i],
