...
#ggsave(file=paste0(outfile.base,'-seq_diag_inf_2014-2018_CIs_sens_midpoint_SC.png'),plot,w=8, h=9)
	...
	#ggsave(file=paste0(outfile.base,'-seq_diag_inf_2014-2018_CIs_q40.png'),plot,w=8, h=9)

saveRDS(pct,file=paste0(outfile.base,'-seq_diag_inf_2014-2018_pct.RDS'))

## make plot of diagnosed and infected by Dutch/foreign-born ----
	
undg <- data.table(reshape2::melt(undg,id.vars=c('trsm','mg_fb','qlabel')))
undg <- dcast(undg,trsm+mg_fb+variable~qlabel,value.var='value')
undg[, varlab:='Estimated \ndiagnosed \nand infected in 2014-2018']
undg[variable=='N_inf', varlab:='Estimated \nundiagnosed, \ndiagnosed \nand infected in 2014-2018']
undg[, varlab2:='Estimated \nundiagnosed, \ndiagnosed \nand infected in 2014-2018']

undg[, mg_fb:= factor(mg_fb,levels=c('0','1','All'),labels=c('Dutch-born','Foreign-born','All'))]
undg[, trsm:=factor(trsm,levels=c('MSM','HSX'),labels=c('Amsterdam MSM','Amsterdam heterosexuals'))]

col_pal = rev(pal_npg("nrc")(2))
	
plot <- ggplot(data=subset(undg,mg_fb!='All'),aes(x=varlab,y=M,fill=variable)) +
		geom_bar(stat='identity', position = "dodge") +
		geom_bar(data=subset(undg,mg_fb!='All' & varlab=='Estimated \ndiagnosed \nand infected in 2014-2018'), aes(x=varlab2,y=M),fill=col_pal[1],stat='identity', position = "identity") +
		#geom_text(aes(label=lab_u), position=position_dodge(width=0.9), vjust=1.1,size=6,colour="black") +
		#scale_y_sqrt(breaks=seq(0,600,50)) +
		facet_grid(trsm~mg_fb,scales="free_y") +
	...
	ggsave(file=paste0(outfile.base,'-diag_inf_2014-2019_mwmb_nl.png'),plot,w=14, h=8)
