...
if(0){
	...
	#args_dir[['out_dir']] <- '/Users/alexb/Box Sync/Roadmap/RQ1 Estimating introductions/update_manscript_BnonB'   
	args_dir[['job_tag']] <- 'undiagnosed_untilmay_2014-2018_HSX'
	args_dir[['trsm']] <- 'HSX'
...
p1 <- ggplot(data=subset(data3)) +
	...
	labs(x="Estimated date of infection",y="Phylogenetically observed transmission clusters",col="") +
...
ggsave(paste0(outfile.base,'-fig2_panel1_sidebyside_order_date_wide.png'),p1,w=20, h=24)
...
p2 <- ggplot(data=subset(data4,MAXD>=2014)) +
	...
	#scale_y_discrete(expand=c(0,0), breaks=c(levels(data3$SG_id)[c(rep(F, 19),T)],200),labels=c(levels(data3$SG_id)[c(rep(F, 19),T)],200),drop=FALSE) +
	facet_grid(trsm~., space='free_y',scales='free_y') +
	...
	theme(axis.text.x=element_text(angle=60, vjust = 0, hjust=0),
		strip.background=element_blank(),
		#strip.text = element_text(), #element_blank(),
		panel.grid.minor = element_blank(),
		panel.grid.major = element_blank(),
		axis.ticks.y = element_blank(),
		panel.background = element_blank(),
...
ggsave(paste0(outfile.base,'-fig2_2panels_2014-2019_order_date_cluster.png'),w=30, h=30)
ggsave(paste0(outfile.base,'-fig2_2panels_2014-2019_order_date_cluster.pdf'),w=30, h=30)
ggsave(paste0(outfile.base,'-fig2_2panels_2014-2019_order_date_cluster_newcolourscheme.pdf'),w=30, h=30)
