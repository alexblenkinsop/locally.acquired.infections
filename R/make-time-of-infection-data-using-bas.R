...
args <- list( 
	...
	analysis= 'analysis_220331',
	indir= '/Users/alexb/Box Sync/Roadmap'
)
`%notin%` <- Negate(`%in%`)

#home <- '/rds/general/project/ratmann_roadmap_data_analysis/live'
home <- '/Users/alexb/Box Sync/Roadmap/RQ1 Estimating introductions'

country_continent <- file.path(home,'misc','country_continent.csv')
infile.indinfo <- file.path(args$indir,'Data','data_220331_ROADMAP','SHM_2201_ROADMAP_220331_tblBAS.csv')

dbas <- read.csv(infile.indinfo,header=T)
dbas <- subset(dbas,REG_EVR_AMSTERDAM==1,select=c(PATIENT,GENDER,ORIGIN,MODE,BIRTH_D,RECART_D,HIV1_POS_D,HIV1_NEG_D,AIDS_D))

dbas[,'HIV1_POS_D'] <- as.Date(dbas[,'HIV1_POS_D'],format="%Y-%m-%d")
dbas[,'HIV1_NEG_D'] <- as.Date(dbas[,'HIV1_NEG_D'],format="%Y-%m-%d")
...
dbas[, GENDER:= factor(GENDER,levels=c(1,2,9),labels=c('Male','Female','Unknown'))]
dbas[,TRANSM:='Other']
dbas[MODE==99,TRANSM:='Unknown']
dbas[MODE==1,TRANSM:='MSM']
dbas[MODE==6,TRANSM:='HSX']
set(dbas,NULL,'MODE',NULL)
dbas[is.na(ORIGIN), ORIGIN:='NA']
dt <- dbas
...
dgeo <- data.table(read.csv(country_continent,header=T))
# recode NAs
dgeo[is.na(iso.3166.country), iso.3166.country:='NA']
dgeo[is.na(continent.code), continent.code:='NA']
...
# add cd4/vl data
infile.cd4 <-	file.path(args$indir, 'Data', 'data_220331_ROADMAP/SHM_2201_ROADMAP_220331_tblLAB_CD4.csv')
infile.rna <-	file.path(args$indir, 'Data', 'data_220331_ROADMAP/SHM_2201_ROADMAP_220331_tblLAB_RNA.csv')
...
dat <- rbind(dat,tmp)
# just keep Amsterdam patients
dat <- dat[dat$PATIENT %in% dt$PATIENT,]
...
set(do, NULL, c('ORIGIN','continent.code','fifteen','keep', 'arttime'), NULL)
